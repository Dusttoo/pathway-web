...
export function AbilitySkillStep({ state, update, onNext, onBack }: StepProps) {
  ...
  // Skills auto-granted by the class (rank >= 1 in initial_proficiencies)
  const classGrantedSkills = new Set(
    SKILL_LIST.filter((s) => (classInitialProfs[s.key] ?? 0) >= 1).map((s) => s.key)
  );

  // Background-granted skill (rank 1, also locked)
  ...
  function updateCustomFeat(
    index: number,
    patch: Partial<{ name: string; featType: string; level: number; details: string }>
  ) {
    update({
      customFeats: customFeats.map((feat, i) => (i === index ? { ...feat, ...patch } : feat)),
    });
  }

  function updateCustomAttack(
    index: number,
    patch: Partial<{ name: string; bonus: string; damage: string; traits: string }>
  ) {
    update({
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <span
            className={`text-xs px-2 py-0.5 rounded-full ${remaining > 0 ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
          >
            {remaining > 0
        ...
        <p className="text-xs text-muted-foreground mb-3">
          Class- and background-granted skills are pre-selected. Choose <strong>{freePicks}</strong>{" "}
          free skill{freePicks !== 1 ? "s" : ""} to train
          {intMod !== 0 && (
            <span className="text-muted-foreground">
              {" "}
              (class base {classTrainedCount} {intMod > 0 ? "+" : "−"} INT {Math.abs(intMod)})
            </span>
          )}
          .
        </p>
        {remaining > 0 && (
          <p className="mb-3 text-xs text-amber-500">
            Select {remaining} more trained skill{remaining !== 1 ? "s" : ""} to continue.
          </p>
        )}
          ...
          {SKILL_LIST.map(({ key, label }) => {
            ...
            return (
              ...
              >
                ...
                {badge && <span className="text-[10px] opacity-70 shrink-0">{badge}</span>}
              </button>
            );
          })}
        </div>
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Add unlimited homebrew, lore, campaign, or vehicle skills beyond the class picks
              above.
            </p>
          </div>
          <button
            type="button"
            onClick={() =>
              update({ additionalSkills: [...additionalSkills, { name: "", rank: 1 }] })
            }
          ...
          <button
            ...
            onClick={() =>
              update({
                customFeats: [
                  ...customFeats,
                  { name: "", featType: "Ancestry", level, details: "" },
                ],
              })
            }
            className="btn-outline text-sm flex items-center gap-1.5"
          ...
          <div className="space-y-2">
            {customFeats.map((feat, index) => (
              <div key={index} className="space-y-3 rounded-lg border border-border p-3">
                <div className="grid grid-cols-[1fr_150px_100px_auto] gap-2 items-center">
                  <input
                    className="input text-sm"
                    value={feat.name}
                  ...
                  <input
                    ...
                    value={feat.level}
                    onChange={(e) =>
                      updateCustomFeat(index, { level: Math.max(1, Number(e.target.value) || 1) })
                    }
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      update({ customFeats: customFeats.filter((_, i) => i !== index) })
                    }
                    className="p-2 rounded-md text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors"
                    title="Remove feat"
                  >
                ...
                <textarea
                  ...
                  value={feat.details}
                  onChange={(e) => updateCustomFeat(index, { details: e.target.value })}
                  placeholder="Describe what this feat does, including action cost, frequency, trigger, benefit, and special rules."
                />
              </div>
            ))}
          </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Add unlimited custom features, ancestry powers, granted abilities, or other character
              specials.
            </p>
          </div>
          <button
            ...
            {customSpecials.map((special, index) => (
              <div key={index} className="grid grid-cols-[1fr_auto] gap-2 items-start">
                <textarea
                  ...
                  onChange={(e) =>
                    update({
                      customSpecials: customSpecials.map((item, i) =>
                ...
                <button
                  type="button"
                  onClick={() =>
                    update({ customSpecials: customSpecials.filter((_, i) => i !== index) })
                  }
                  className="p-2 rounded-md text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors"
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Add unlimited strikes, spell attacks, special attacks, or campaign-specific attack
              options.
            </p>
          </div>
          <button
            ...
            onClick={() =>
              update({
                customAttacks: [...customAttacks, { name: "", bonus: "", damage: "", traits: "" }],
              })
            }
            className="btn-outline text-sm flex items-center gap-1.5"
          >
              ...
              <div key={index} className="border border-border rounded-lg p-3 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      update({ customAttacks: customAttacks.filter((_, i) => i !== index) })
                    }
