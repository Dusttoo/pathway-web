...
export default function ClassDetailPage() {
  ...
  const { data: cls, isLoading, error } = useQuery<CharacterClass, Error>({
  ...
  const features = Array.isArray(cls.class_features) ? (cls.class_features as string[]) : [];
  ...
  return (
    <MainLayout>
      <div className="space-y-6">
        ...
        {cls.source && (
          <p className="text-xs text-muted-foreground">Source: {cls.source}</p>
        )}
      </div>
    </MainLayout>
  );
