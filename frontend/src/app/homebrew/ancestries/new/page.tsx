...
export default function NewAncestryPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const ancestryData = {
      ...
      ability_boosts: abilityBoostMode === "Two free boosts" ? ["free", "free"] : listFromComma(abilityBoosts),
  ...
  return (
    <MainLayout>
      <div className="max-w-3xl space-y-6">
        <Link href="/homebrew?tab=ancestry" className="inline-flex items-center gap-2 text-primary hover:text-primary/80 text-sm">
          <ArrowLeft size={14} />
          Back to Homebrew
        </Link>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="card p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-6 items-start">
              <Field label="Ancestry Name" required>
                <input className="input" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Stormmarked Elf" required />
              </Field>
          ...
          <div className="card p-6 space-y-4">
            <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Core Statistics</h2>
            <div className="grid grid-cols-3 gap-4">
              <Field label="Hit Points">
                <input type="number" className="input" value={hp} onChange={(e) => setHp(e.target.value)} min={1} />
              </Field>
              <Field label="Size">
                <select className="input" value={size} onChange={(e) => setSize(e.target.value)}>
                  {["Tiny", "Small", "Medium", "Large"].map((value) => <option key={value}>{value}</option>)}
                </select>
              </Field>
              <Field label="Speed">
                <input type="number" className="input" value={speed} onChange={(e) => setSpeed(e.target.value)} min={0} />
              </Field>
            </div>
            <Field label="Ability Boosts">
              <select className="input" value={abilityBoostMode} onChange={(e) => setAbilityBoostMode(e.target.value as AbilityBoostMode)}>
                {(["Two free boosts", "Fixed boosts + free boost", "Custom"] as const).map((value) => <option key={value}>{value}</option>)}
              </select>
            </Field>
            {abilityBoostMode !== "Two free boosts" && (
              <Field label="Boost Details" hint="Comma-separated, e.g. Dexterity, Intelligence, Free">
                <input className="input" value={abilityBoosts} onChange={(e) => setAbilityBoosts(e.target.value)} />
              </Field>
            )}
            <Field label="Ability Flaw">
              <input className="input" value={abilityFlaw} onChange={(e) => setAbilityFlaw(e.target.value)} placeholder="Optional" />
            </Field>
          </div>

          <div className="card p-6 space-y-4">
            <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Languages & Senses</h2>
            ...
            <Field label="Additional Languages">
              <input className="input" value={additionalLanguages} onChange={(e) => setAdditionalLanguages(e.target.value)} placeholder="Equal to Intelligence modifier from this list..." />
            </Field>
            <Field label="Senses">
              <input className="input" value={senses} onChange={(e) => setSenses(e.target.value)} placeholder="Low-light vision, darkvision" />
            </Field>
          </div>
          ...
          <div className="card p-6 space-y-4">
            ...
            <Field label="Ancestry Features">
              <textarea className="input min-h-[100px] resize-y" value={features} onChange={(e) => setFeatures(e.target.value)} placeholder="Special rules, natural attacks, resistances, or other ancestry features." />
            </Field>
          ...
          <div className="flex items-center gap-3">
            <button type="submit" disabled={createHomebrew.isPending} className="btn-primary flex items-center gap-2">
              {createHomebrew.isPending ? <><div className="spinner w-4 h-4" />Saving...</> : <><Users size={16} />Create Ancestry</>}
            </button>
