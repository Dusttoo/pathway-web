...
function buildMonsterData(fields: {
  name: string; level: string; size: Size; rarity: Rarity; traits: string;
  hp: string; ac: string; perception: string; fort: string; ref: string; will: string;
  speed: string; flySpeed: string; burrowSpeed: string; swimSpeed: string; climbSpeed: string;
  senses: string; languages: string; description: string;
  ...
  const rich = {
    ...
    ability_modifiers: {
      str: parseMod(fields.strMod), dex: parseMod(fields.dexMod),
    ...
    defenses: {
      ...
      immunities: fields.immunities
        ? fields.immunities.split(",").map((s) => s.trim()).filter(Boolean)
        : [],
...
export default function NewMonsterPage() {
  ...
  async function handleFormSubmit(e: React.FormEvent) {
    e.preventDefault();
    setFormError(null);
    if (!name.trim()) { setFormError("Monster name is required."); return; }
    if (!hp || !ac)   { setFormError("HP and AC are required."); return; }

    const built = buildMonsterData({
      name: name.trim(), level, size, rarity, traits, hp, ac, perception,
      fort, ref, will,
      speed, flySpeed, burrowSpeed, swimSpeed, climbSpeed,
      senses, languages, description,
      strMod, dexMod, conMod, intMod, wisMod, chaMod,
      skills,
      immunities, hpNotes, weaknesses, resistances,
      items, attacks, abilities,
    });
    ...
    try {
      await createHomebrew.mutateAsync({
        type: "monster",
        name: name.trim(),
        data: { name: built.core.name, core: built.core, rich: built.rich, summary: built.summary, image_url: imageUrl },
      });
  ...
  async function handleJsonSubmit(e: React.FormEvent) {
    ...
    if (!entryName) { setFormError('JSON must have a top-level "name" field (or core.name).'); return; }
    try {
  ...
  return (
    <MainLayout>
      <div className="max-w-3xl space-y-6">
        {/* Back */}
        <Link href="/homebrew?tab=monster"
        ...
        <div className="flex gap-1 p-1 bg-muted/40 rounded-lg border border-border w-fit">
          {([["form", LayoutList, "Form Builder"], ["json", FileCode, "JSON Import"]] as const).map(([m, Icon, label]) => (
          ...
          <form onSubmit={handleFormSubmit} className="space-y-6">
            ...
            <div className="card p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-6 items-start">
                <Field label="Name" required>
                  <input type="text" className="input" value={name} onChange={(e) => setName(e.target.value)}
                    placeholder="e.g. Ember Drake" required />
                </Field>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <Field label="Size">
                  <select className="input" value={size} onChange={(e) => setSize(e.target.value as Size)}>
                    {(["Tiny","Small","Medium","Large","Huge","Gargantuan"] as const).map((s) => <option key={s}>{s}</option>)}
                  </select>
                </Field>
                <Field label="Rarity">
              ...
              <Field label="Traits" hint="Comma-separated, e.g. dragon, fire, beast">
                <input type="text" className="input" value={traits} onChange={(e) => setTraits(e.target.value)} placeholder="dragon, fire" />
              </Field>
            </div>

            {/* Defenses */}
            <div className="card p-6 space-y-4">
              <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Defenses</h2>
              <div className="grid grid-cols-3 gap-4">
                <Field label="HP" required>
                  <input type="number" className="input" value={hp} onChange={(e) => setHp(e.target.value)} placeholder="120" min={0} required />
                </Field>
                <Field label="AC" required>
                  <input type="number" className="input" value={ac} onChange={(e) => setAc(e.target.value)} placeholder="22" min={0} required />
                </Field>
                <Field label="Perception">
                  <input type="number" className="input" value={perception} onChange={(e) => setPerception(e.target.value)} placeholder="+12" />
                </Field>
              </div>
              <div className="grid grid-cols-3 gap-4">
                <Field label="Fortitude">
                  <input type="number" className="input" value={fort} onChange={(e) => setFort(e.target.value)} placeholder="+14" />
                </Field>
                <Field label="Reflex">
                  <input type="number" className="input" value={ref} onChange={(e) => setRef(e.target.value)} placeholder="+10" />
                </Field>
                <Field label="Will">
                  <input type="number" className="input" value={will} onChange={(e) => setWill(e.target.value)} placeholder="+9" />
                </Field>
              </div>
              ...
              <div className="space-y-2">
                ...
                <button type="button" onClick={() => addDefenseRow(setWeaknesses, { type: "", value: "" })}
              ...
              <div className="space-y-2">
                ...
                <button type="button" onClick={() => addDefenseRow(setResistances, { type: "", value: "" })}
                  className="btn-outline text-sm flex items-center gap-1.5">
            ...
            {/* Ability scores */}
            <div className="card p-6 space-y-4">
              <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Ability Modifiers</h2>
              <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
                {([
                  ["STR", strMod, setStrMod], ["DEX", dexMod, setDexMod], ["CON", conMod, setConMod],
                  ["INT", intMod, setIntMod], ["WIS", wisMod, setWisMod], ["CHA", chaMod, setChaMod],
                ] as [string, string, (v: string) => void][]).map(([label, val, setter]) => (
                  <div key={label}>
                    <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1 text-center">{label}</label>
                    <input type="number" className="input text-center" value={val}
                      onChange={(e) => setter(e.target.value)} placeholder="+0" />
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Skills */}
            <div className="card p-6 space-y-4">
              <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Skills</h2>
              {skills.length === 0 && <p className="text-sm text-muted-foreground">No skills added yet.</p>}
              <datalist id="pf2e-skills">
                {PF2E_SKILLS.filter((s) => s !== "Lore (custom)").map((s) => <option key={s} value={s} />)}
              ...
              <div className="space-y-2">
                {skills.map((row, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <input type="text" list="pf2e-skills" className="input flex-1" placeholder="e.g. Stealth"
                      value={row.name}
                      onChange={(e) => { const n = [...skills]; n[i] = { ...n[i], name: e.target.value }; setSkills(n); }} />
                    <div className="flex items-center gap-1 shrink-0">
                      <span className="text-muted-foreground text-sm">+</span>
                      <input type="number" className="input w-20 text-center" placeholder="0"
                        value={row.bonus}
                        onChange={(e) => { const n = [...skills]; n[i] = { ...n[i], bonus: e.target.value }; setSkills(n); }} />
                    </div>
              ...
              <button type="button" onClick={() => setSkills([...skills, { name: PF2E_SKILLS[0], bonus: "" }])}
                className="btn-outline text-sm flex items-center gap-1.5">
            ...
            {/* Movement */}
            <div className="card p-6 space-y-4">
              <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Movement, Senses &amp; Languages</h2>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <Field label="Land Speed (ft)">
                  <input type="number" className="input" value={speed} onChange={(e) => setSpeed(e.target.value)} placeholder="25" min={0} />
                </Field>
              ...
              <Field label="Senses" hint="Comma-separated, e.g. darkvision, scent (imprecise) 30 feet">
                <input type="text" className="input" value={senses} onChange={(e) => setSenses(e.target.value)} placeholder="darkvision" />
              </Field>
              <Field label="Languages" hint="Comma-separated, e.g. Draconic, Common">
                <input type="text" className="input" value={languages} onChange={(e) => setLanguages(e.target.value)} placeholder="Draconic" />
              </Field>
            </div>
            ...
            {/* Attacks */}
            <div className="card p-6 space-y-4">
              <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Attacks</h2>
              ...
              <button type="button"
                onClick={() => addDefenseRow(setAttacks, { kind: "Melee", name: "", bonus: "", traits: "", damage: "" })}
                className="btn-outline text-sm flex items-center gap-1.5">
            ...
            {/* Abilities */}
            <div className="card p-6 space-y-4">
              <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Special Abilities</h2>
              ...
              <button type="button"
                onClick={() => addDefenseRow(setAbilities, { name: "", cost: "2 Actions", traits: "", description: "" })}
                className="btn-outline text-sm flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-3">
              <button type="submit" disabled={createHomebrew.isPending} className="btn-primary flex items-center gap-2">
                {createHomebrew.isPending ? <><div className="spinner w-4 h-4" />Saving…</> : <><Swords size={16} />Create Monster</>}
              </button>
        ...
        {mode === "json" && (
          <form onSubmit={handleJsonSubmit} className="space-y-6">
            <div className="card p-6 space-y-4">
              <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">JSON Import</h2>
              <p className="text-sm text-muted-foreground">
                Paste a complete monster JSON object. Must include a top-level{" "}
                <code className="text-xs bg-muted px-1 py-0.5 rounded">name</code> field. Ideally includes{" "}
              ...
              <textarea className="input min-h-[320px] resize-y font-mono text-xs" value={jsonText}
            ...
            <div className="flex items-center gap-3">
              <button type="submit" disabled={createHomebrew.isPending} className="btn-primary flex items-center gap-2">
                {createHomebrew.isPending ? <><div className="spinner w-4 h-4" />Saving…</> : <><FileCode size={16} />Import Monster</>}
              </button>
              <Link href="/homebrew?tab=monster" className="btn-outline">Cancel</Link>
            </div>
          </form>
