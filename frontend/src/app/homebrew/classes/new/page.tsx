  ...
  return (
    <MainLayout>
      <div className="max-w-3xl space-y-6">
        <Link href="/homebrew?tab=class" className="inline-flex items-center gap-2 text-primary hover:text-primary/80 text-sm">
          <ArrowLeft size={14} />
          Back to Homebrew
        </Link>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="card p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-6 items-start">
              <Field label="Class Name" required>
                <input className="input" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Stormcaller" required />
              </Field>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="Key Ability" hint="Comma-separated if multiple">
                <input className="input" value={keyAbility} onChange={(e) => setKeyAbility(e.target.value)} placeholder="Strength, Dexterity" />
              </Field>
            </div>
          </div>

          <div className="card p-6 space-y-4">
            <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Initial Proficiencies</h2>
            <div className="grid grid-cols-2 gap-4">
              <Field label="HP Per Level">
                <input type="number" className="input" value={hpPerLevel} onChange={(e) => setHpPerLevel(e.target.value)} min={1} />
              </Field>
              <Field label="Perception">
                <select className="input" value={perception} onChange={(e) => setPerception(e.target.value)}>
                  {["Untrained", "Trained", "Expert", "Master", "Legendary"].map((value) => <option key={value}>{value}</option>)}
                </select>
              </Field>
            </div>
            <Field label="Saving Throws">
              <input className="input" value={savingThrows} onChange={(e) => setSavingThrows(e.target.value)} placeholder="Fortitude expert, Reflex trained, Will expert" />
            </Field>
            <Field label="Skills">
              <textarea className="input min-h-[80px] resize-y" value={skills} onChange={(e) => setSkills(e.target.value)} placeholder="Trained in Arcana, plus a number of additional skills..." />
            </Field>
            <Field label="Attacks">
              <input className="input" value={attacks} onChange={(e) => setAttacks(e.target.value)} placeholder="Simple weapons trained, martial weapons trained" />
            </Field>
            <Field label="Defenses">
              <input className="input" value={defenses} onChange={(e) => setDefenses(e.target.value)} placeholder="Unarmored defense trained, light armor trained" />
            </Field>
            <div className="grid grid-cols-2 gap-4">
              <Field label="Class DC">
                <select className="input" value={classDc} onChange={(e) => setClassDc(e.target.value)}>
                  {["Untrained", "Trained", "Expert", "Master", "Legendary"].map((value) => <option key={value}>{value}</option>)}
                </select>
              </Field>
              <Field label="Spellcasting Tradition">
                <input className="input" value={tradition} onChange={(e) => setTradition(e.target.value)} placeholder="Arcane, divine, occult, primal" />
              </Field>
            </div>
          </div>
          ...
          <div className="card p-6 space-y-4">
            ...
            <Field label="Class Features">
              <textarea className="input min-h-[120px] resize-y" value={features} onChange={(e) => setFeatures(e.target.value)} placeholder="List level 1 features, class paths, special rules, or class-defining mechanics." />
            </Field>
          ...
          <div className="flex items-center gap-3">
            <button type="submit" disabled={createHomebrew.isPending} className="btn-primary flex items-center gap-2">
              {createHomebrew.isPending ? <><div className="spinner w-4 h-4" />Saving...</> : <><GraduationCap size={16} />Create Class</>}
            </button>
